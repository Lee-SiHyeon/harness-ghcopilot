...
export function registerPipelineTools(server) {
    ...
    server.registerTool("pipeline_record_start", { description: "Record a subagent start event into last-subagent-start.json", inputSchema: z.object({
    ...
    server.registerTool("pipeline_record_stop", { description: "Record a subagent stop event, calculate duration, and append to subagent-flow.jsonl", inputSchema: z.object({
        ...
        if (!entry) {
            return { content: [{ type: "text", text: JSON.stringify({ error: "No matching start entry found" }) }] };
        }
        return { content: [{ type: "text", text: JSON.stringify(entry, null, 2) }] };
    });
    server.registerTool("pipeline_query", { description: "Query recent entries from subagent-flow.jsonl", inputSchema: z.object({
