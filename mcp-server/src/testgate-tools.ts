...
export function registerTestGateTools(server: McpServer): void {
  const testgate = loadStateLib("testgate");

  server.registerTool(
    "testgate_get",
    { description: "Get the current test gate state from test-gate-state.json", inputSchema: z.object({}) },
    async () => {
  ...
  server.registerTool(
    "testgate_set",
    { description: "Partially update test-gate-state.json", inputSchema: z.object({
  ...
  server.registerTool(
    ...
    { description: "Write test evidence to test-evidence.json", inputSchema: z.object({
      evidence: z.object({
  ...
  server.registerTool(
    "testgate_is_valid",
    { description: "Check if the current test evidence is valid (status=PASS and ts >= requiredSince)", inputSchema: z.object({}) },
    async () => {
