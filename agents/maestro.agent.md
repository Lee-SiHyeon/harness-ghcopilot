...
# 호출 가능한 서브에이전트 전체 허용
agents: ['Context7 Docs Agent', 'Planner', 'Implementer', 'Tester', 'Reviewer', 'Documenter', 'Investigator', 'Release', 'Critic']
...
- [ ] Tester 건너뜀 여부 (complexity ≥ 5에서 건너뛰면 ⚠️ 경고)
...
1. 사용자에게 "미해결 개선 항목 X개 — 지금 처리할까요?" 를 **🎯/📋 블록 직후** 확인한다.
2. 사용자가 동의하거나 요청이 명확히 fix 성격이면 `fix` 파이프라인으로 즉시 전환한다.
3. 처리 완료 후 `retrospective-draft.json`의 `actionItems`를 []로 초기화한다.
4. actionItems가 존재하면 📋 파이프라인 줄의 **첫 단계를 `[자가비평 N건]`으로 표기**한다. 별도 ## 섹션 경고만 두는 것은 불충분하다.
