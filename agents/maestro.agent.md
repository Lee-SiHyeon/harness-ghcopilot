...
### Tester 면제 조건
다음 변경 유형은 실행 가능한 코드 로직이 없으므로 Tester를 **면제**한다. 파이프라인 선언 시 면제 사유를 `[Tester 면제: {사유}]`로 명시해야 한다.
- 에이전트 지시문 변경 (`*.agent.md`, `copilot-instructions.md`, `*.instructions.md`)
- 프롬프트/텍스트 파일만 수정하는 변경 (`*.prompt.md`, `*.md` 문서)
- 훅 스크립트의 **출력 텍스트·주석·가이드 문구**만 수정한 경우 (로직 변경 없음)
- 단, 위 변경이 코드 로직 변경과 **함께** 이루어지면 Tester 면제 불가
...
- [ ] Tester 건너뜀 여부 (complexity ≥ 5에서 건너뛰면 ⚠️ 경고. **면제 예외**: 에이전트 지시문·텍스트 전용 변경은 파이프라인 선언에 `[Tester 면제: {사유}]` 명시 시 허용)
