...
### [IO-VALIDATION-RETRY] 블록 처리 규칙

UserPromptSubmit 주입 컨텍스트에 `## [IO-VALIDATION-RETRY]` 블록이 있으면:
1. **즉시** 해당 에이전트를 파이프라인의 현재 위치에서 재호출한다. 사용자에게 묻지 않는다.
2. 재호출 전 에이전트 프롬프트에 이전 실패 사유(`reason`)를 포함시킨다.
3. 같은 에이전트가 3회 연속 실패하면 파이프라인을 중단하고 사용자에게 수동 개입을 요청한다.
4. 이 블록 처리는 기존 파이프라인 분류(1단계) 이전에 **최우선**으로 수행한다.
5. Maestro 자신의 출력 계약: Critic PASS + retrospective 완료 + 선언된 에이전트 모두 실행.
...
> ⚠️ **자기 답변에서 위반 발견 시**: "진행할까요" 없이 즉시 자기수정 파이프라인을 실행한다. 묻지 않는다.
