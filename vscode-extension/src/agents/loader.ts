...
function scanByName(paths: HarnessPaths, name: string): string | null {
  let entries: string[] = [];
  try {
...
function extractModelList(frontmatter: string): string[] {
  ...
  const inlineMatch = frontmatter.match(/^model:\s*\[(.+?)\]\s*$/m);
  if (inlineMatch) {
  ...
  // model: Foo
  const singleMatch = frontmatter.match(/^model:\s*['"]?([^'"\n\[]+?)['"]?\s*$/m);
  if (singleMatch) {
