...
tc('tc-113', 'file-guard / Maestro logs allow', 'AGENT_NAME=Maestro + .github/logs/retro.jsonl 경로 → deny 아님(continue: true) 검증', () => {
  const guardPath = path.join(HOOKS, 'file-guard.js');
  const env = {
    ...process.env,
    TOOL_NAME: 'create_file',
    AGENT_NAME: 'Maestro',
    SUBAGENT_NAME: '',
    TOOL_INPUT: JSON.stringify({ path: path.resolve('.github/logs/retro.jsonl') }),
  };
  const raw = execSync(`node "${guardPath}"`, { env, stdio: ['pipe', 'pipe', 'pipe'] }).toString().trim();
  const result = JSON.parse(raw);
  if (result.continue === false && result.decision === 'deny') throw new Error(`Maestro + retro.jsonl path should be allowed, got: ${JSON.stringify(result)}`);
  if (result.decision === 'deny') throw new Error(`Expected allow for retro.jsonl path, got deny: ${JSON.stringify(result)}`);
});
...
// ════════════════════════════════════════════════════════════════
// GROUP: maestro-router / query-disclosure
// ════════════════════════════════════════════════════════════════
tc('tc-149', 'maestro-router / query-disclosure', '단순 질문("이게 뭐야?")도 🎯 작업 유형 + 📋 파이프라인 헤더 포함', () => {
  const result = runMaestroRouter('이게 뭐야?');
  const userMessage = result.modifiedParameters && result.modifiedParameters.userMessage;
  if (!userMessage) throw new Error(`modifiedParameters.userMessage 없음: ${JSON.stringify(result)}`);
  if (!userMessage.includes('🎯 **작업 유형**')) {
    throw new Error(`🎯 **작업 유형** 헤더 누락: ${userMessage}`);
  }
  if (!userMessage.includes('📋 **파이프라인**')) {
    throw new Error(`📋 **파이프라인** 헤더 누락: ${userMessage}`);
  }
...
// ════════════════════════════════════════════════════════════════
// GROUP: maestro-policy / query-always-disclose
// ════════════════════════════════════════════════════════════════
tc('tc-150', 'maestro-policy / query-always-disclose', 'maestro.agent.md — query 포함 모든 유형 예외 없이 헤더 주입 명시', () => {
  const md = readAgent('maestro.agent.md');
  if (!/(query 포함|단순 질문|모든 유형).*예외 없/.test(md)) {
    throw new Error('query 포함 모든 유형 예외 없음 정책 문구 없음');
  }
  if (!md.includes('🎯')) throw new Error('🎯 기호 언급 없음');
  if (!md.includes('📋')) throw new Error('📋 기호 언급 없음');
});

tc('tc-151', 'maestro-policy / no-confirm-gate', 'maestro.agent.md — "진행할까요" 없이 즉시/자동으로 파이프라인 실행 명시', () => {
  const md = readAgent('maestro.agent.md');
  if (!/"진행할까요".*없이|진행할까요.*금지/.test(md)) {
    throw new Error('"진행할까요" 없이 또는 금지 문구 없음');
  }
  if (!/(즉시|자동으로).*파이프라인/.test(md)) {
    throw new Error('즉시/자동으로 파이프라인 실행 문구 없음');
  }
});

tc('tc-152', 'maestro-policy / violation-auto-fix', 'maestro.agent.md — 위반 즉시 파이프라인 재실행 또는 자동수정 정책 명시', () => {
  const md = readAgent('maestro.agent.md');
  if (!/위반.*즉시|즉시.*파이프라인|위반.*자동수정/.test(md)) {
    throw new Error('위반 즉시/자동수정 정책 문구 없음');
  }
});

// ════════════════════════════════════════════════════════════════
// GROUP: file-guard / retrospective-history deny
// ════════════════════════════════════════════════════════════════
tc('tc-153', 'file-guard / retrospective-history deny', 'AGENT_NAME=Maestro + retrospective-history.md → deny', () => {
  const guardPath = path.join(HOOKS, 'file-guard.js');
  const env = {
    ...process.env,
    TOOL_NAME: 'edit_file',
    AGENT_NAME: 'Maestro',
    SUBAGENT_NAME: '',
    TOOL_INPUT: JSON.stringify({ filePath: '.github/logs/retrospective-history.md' }),
  };
  const raw = execSync(`node "${guardPath}"`, { env, stdio: ['pipe', 'pipe', 'pipe'] }).toString().trim();
  const result = JSON.parse(raw);
  if (result.decision !== 'deny') {
    throw new Error(`Expected deny for retrospective-history.md, got: ${JSON.stringify(result)}`);
  }
});

// ════════════════════════════════════════════════════════════════
// GROUP: maestro-policy / file-write-rules
// ════════════════════════════════════════════════════════════════
tc('tc-154', 'maestro-policy / retro-write-rule', 'maestro.agent.md — retrospective-history.md는 Implementer 경유, retro.jsonl은 Maestro 직접 명시', () => {
  const md = readAgent('maestro.agent.md');
  if (!md.includes('retrospective-history.md')) {
    throw new Error('retrospective-history.md 언급 없음');
  }
  if (!/Implementer.*경유|Implementer를 통해/.test(md)) {
    throw new Error('Implementer 경유/통해 문구 없음');
  }
  if (!md.includes('retro.jsonl')) {
    throw new Error('retro.jsonl 언급 없음');
  }
  if (!/Maestro 직접 가능|Maestro가 직접/.test(md)) {
    throw new Error('Maestro 직접 가능/직접 문구 없음');
  }
});

tc('tc-155', 'maestro-policy / test-required-scope', 'maestro.agent.md — .github/agents/ 또는 hooks/ 변경 시 maestro-suite.test.js 실행 필수 명시', () => {
  const md = readAgent('maestro.agent.md');
  if (!/\.github\/agents\/|\.github\/hooks\//.test(md)) {
    throw new Error('.github/agents/ 또는 .github/hooks/ 경로 언급 없음');
  }
  if (!md.includes('maestro-suite.test.js')) {
    throw new Error('maestro-suite.test.js 언급 없음');
  }
  if (!/필수|반드시/.test(md)) {
    throw new Error('필수/반드시 문구 없음');
  }
});

tc('tc-156', 'maestro-policy / test-file-extension-agnostic', 'maestro.agent.md — maestro-suite.test.js 언급 근처 5줄 이내에 확장자 무관/모든 파일 문구 포함', () => {
  const md = readAgent('maestro.agent.md');
  const lines = md.split('\n');
  const testFileIdx = lines.findIndex(l => l.includes('maestro-suite.test.js'));
  if (testFileIdx === -1) throw new Error('maestro-suite.test.js 언급 없음');
  const window = lines.slice(Math.max(0, testFileIdx - 5), testFileIdx + 6).join('\n');
  if (!/확장자.*무관|모든 파일/.test(window)) {
    throw new Error(`maestro-suite.test.js 근처 5줄 이내에 "확장자 무관" 또는 "모든 파일" 문구 없음:\n${window}`);
  }
});
