...
tc('tc-075', 'maestro-router.js / release-intent-no-double', 'release 인텐트는 Release 중복 없음', () => {
  const r = getClassify()('릴리즈해줘');
  const releaseCount = r.pipeline.filter(a => a === 'Release').length;
  if (releaseCount > 1) throw new Error(`release 파이프라인에 Release가 ${releaseCount}개 — 중복`);
});
